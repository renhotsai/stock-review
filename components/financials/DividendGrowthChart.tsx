...
export default function DividendGrowthChart({ records }: DividendGrowthChartProps) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
      <h3 className="text-base font-bold text-gray-900 mb-4">股息增長趨勢</h3>
      <ResponsiveContainer width="100%" height={220}>
        <ComposedChart data={data} margin={{ top: 0, right: 8, left: 0, bottom: 0 }}>
          ...
          <Bar yAxisId="left" dataKey="年化股息" fill="#3b82f6" radius={[4, 4, 0, 0]} opacity={0.8} />
          <Line yAxisId="right" type="monotone" dataKey="同比增長率" stroke="#f59e0b" strokeWidth={2} dot={{ r: 3 }} />
        </ComposedChart>
      </ResponsiveContainer>
    </div>
  );
